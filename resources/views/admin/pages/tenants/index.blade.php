            ...
            <table class="table table-condensed">
                ...
                <tbody>
                    @foreach ($tenants as $tenant)
                        <tr>
                            <td>
                                <img src="{{ url("storage/{$tenant->logo}") }}" alt="{{ $tenant->title }}" class="img-circle img-bordered-sm" style="max-width: 90px">
                            </td>
                            <td>{{ $tenant->name }}</td>
                            <td style="width=10px;">
                                <a href="{{ route('tenants.edit', $tenant->id) }}" class="btn btn-info"><i class="fas fa-edit"></i></a>
                                <a href="{{ route('tenants.show', $tenant->id) }}" class="btn btn-success"><i class="fas fa-eye"></i></a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
